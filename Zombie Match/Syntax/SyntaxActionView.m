...
@implementation SyntaxActionView
...
@synthesize isGameOver, gameEngine, isPlaying;
...
- (id)initWithFrame:(CGRect)frame andViewController:(ViewController *)controller {
    ...
    if (self) {
        ...
        self.gameEngine = [[SyntaxEngineAction alloc] initWithFrame:CGRectMake(0, 70, 320, 320) andViewController:viewController];
        ...
        timerBar = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 500, 17)];
        timerBar.backgroundColor = [UIColor colorWithRed:1 green:.5 blue:.1 alpha:1];
        timerBar.center = CGPointMake(50, 9.5);
        timerBar.opaque = YES;
        [progressBarBackground addSubview:timerBar];
        
        progressBar = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 500, 17)];
        progressBar.backgroundColor = [UIColor colorWithRed:.8 green:0.11 blue:0.11 alpha:0.5];
        ...
        // Timed Mode
        timedMode = [[SyntaxLabel alloc] initWithFrame:CGRectMake(0, 0, 100, 28)];
        timedMode.text = @"TIMED MODE";
        timedMode.center = CGPointMake(274, 10);
        [timedMode styleWithSize:12];
        timedMode.textColor = [UIColor colorWithRed:0.5 green:0.5 blue:0.5 alpha:1];
        [self addSubview:timedMode];
...
- (void)showButtons {
    ...
    [self fadeInView:timerBarBackground withAction:^{}];
    ...
    [self fadeInView:powButton withAction:^{}];
    [self fadeInView:timedMode withAction:^{}];
...
- (void)hideButtons {
    ...
    [self fadeOutView:timerBarBackground withAction:^{}];
    ...
    [self fadeOutView:timedMode withAction:^{}];
...
- (void)countdown {
    ...
    [UIView animateWithDuration:1
                     ...
                     animations:^{
                         timerBar.center = CGPointMake(50 - ((90 - timer) * 3.33), 9.5);
                     }
                     completion:^(BOOL finished) {
                         if (isVisible & isPlaying) {
                             self.timer--;
                             if (self.timer > 0) [self countdown];
                             else {
                                 isGameOver = YES;
                                 if (gameEngine.canTouch) [self gameOver];
                             }
                         }
                     }];
}
...
- (void)hideNextLevel {
    gameEngine.isWaitingForLevel = YES;
    [UIView animateWithDuration:0.0333
                          delay:4
                        options:UIViewAnimationOptionAllowUserInteraction
                     animations:^{
...
- (void)removeProposeReshuffle {
    [self.gameEngine setUserInteractionEnabled:YES];
    [self.gameEngine setCanShowStuck:YES];
    [self.gameEngine setAlpha:1];
    [propositionToShuffle removeFromSuperview];
    propositionToShuffle = nil;
    [gameOverButton removeFromSuperview];
    gameOverButton = nil;
...
- (void)gameOverHandler {
    [self removeProposeReshuffle];
    self.userInteractionEnabled = NO;
    [gameEngine clearGame];
    [self clearInfoPopUps];
    [self hideButtons];
    [viewController performSelector:@selector(gameOverWithReason:) withObject:@"Out of time" afterDelay:1];
}
...
- (void)showPurchaseAlert:(int)num {
    ...
    if(num == kPurchClue)
        caption = @"Would you like to buy 500 tokens for $.99 and get 5 Hints?";
...
- (void)reshuffleHandler {
    if(![viewController.byteManager getReshuffle:500]) {
        [viewController.soundController playSound:@"NoMatch"];
        [self showGetBytesAlert];
    }
    else {
        [self.gameEngine setCanShowStuck:YES];
        [self.gameEngine reshuffle];
        [self removeProposeReshuffle];
    }
}
...
- (void)proposeReshuffle {
    ...
    [self.gameEngine setUserInteractionEnabled:NO];
    [self.gameEngine setCanShowStuck:NO];
    [self.gameEngine setAlpha:0.5];
    propositionToShuffle = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"reshuffle.png"]];
    [propositionToShuffle setCenter:CGPointMake(160, 230)];
    [self addSubview:propositionToShuffle];
    [propositionToShuffle release];
    
    gameOverButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 110, 81)];
    [gameOverButton setCenter:CGPointMake(96, 329)];
    [gameOverButton setImage:[UIImage imageNamed:@"reshuffle_endpress.png"] forState:UIControlStateHighlighted];
    [gameOverButton addTarget:self action:@selector(gameOverHandler) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:gameOverButton];
    [gameOverButton release];
    
    reshuffleButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 110, 81)];
    [reshuffleButton setCenter:CGPointMake(207, 328)];
    [reshuffleButton setImage:[UIImage imageNamed:@"reshuffle_press.png"] forState:UIControlStateHighlighted];
    [reshuffleButton addTarget:self action:@selector(reshuffleHandler) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:reshuffleButton];
    [reshuffleButton release];
    
}

- (void)gameOver {
    self.userInteractionEnabled = NO;
    gameEngine.isWaitingForLevel = NO;
    isPlaying = NO;
    [gameEngine clearGame];
    [self clearInfoPopUps];
    [self hideButtons];
...
- (void)restart {
    ...
    timerBar.center = CGPointMake(50, 6);
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    [self checkIfPowerUpsNeedToGlow];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"refreshStuckTimerAction" object:nil];
...
- (void)dealloc {
    ...
    [self removeSubView:timerBar];
    [self removeSubView:timerBarBackground];
