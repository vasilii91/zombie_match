...
//  Syntax
//
//  Created by Seby Moisei on 3/3/12.
...
- (id)initSlide {
    ...
    if (self) {
        ...
        title.text = @"4 symbol match";
        title.center = CGPointMake(160, 60);
        [title styleWithSize:25];
        [self addSubview:title];
        
        slidetext = [[SyntaxLabel alloc] initWithFrame:CGRectMake(0, 0, 320, 80)];
        slidetext.text = @"Matching 4 zombies will get them very excited. \nThey will take all adjacent zombies back to the zoo with them!";
...
- (void)initSymbols {
    ...
    symbolToMakeExplosive = [symbolManager randomSymbolWithMaxType:7];
    symbolToMakeExplosive.center = [self CGPointFromIndex:CGPointMake(0, 1)];
    [symbolManager shiftSymbol:symbolToMakeExplosive toType:3];
    [self addSubview:symbolToMakeExplosive];
...
- (void)step3 {
    [symbolManager turnToExplosiveSymbol:symbolToMakeExplosive];
...
- (void)step5 {
    ...
    symbolA = [symbolManager randomSymbolWithMaxType:7];
    symbolA.center = [self CGPointFromIndex:CGPointMake(1, 1)];
    [symbolManager shiftSymbol:symbolA toType:2];
    [allSymbols addObject:symbolA];
    symbolA.alpha = 0;
    [self addSubview:symbolA];
...
- (void)step6 {
    [symbolManager selectSymbol:symbolToMakeExplosive];
    if (!canRelease) [self performSelector:@selector(step7) withObject:nil afterDelay:0.3];
    else [self releaseSymbols];
}

- (void)step7 {
    [symbolManager deselectSymbol:symbolToMakeExplosive];
    [UIView animateWithDuration:0.5
                          delay:0
                        options:(UIViewAnimationOptionAllowUserInteraction | UIViewAnimationOptionCurveEaseInOut)
                     animations:^{
                         symbolA.center = [self CGPointFromIndex:CGPointMake(0, 1)];
                         symbolToMakeExplosive.center = [self CGPointFromIndex:CGPointMake(1, 1)];
                     }
                     completion:^(BOOL finished) {
...
- (void)step8 {
    [symbolToMakeExplosive retain];
    [symbolManager animExplodeSymbol:symbolToMakeExplosive WithDelay:0];
    for (SyntaxSymbol *thisSymbol in allSymbols) {
        float rand = (float) (arc4random() % 50) / 100;
        [symbolManager animExplodeSymbol:thisSymbol WithDelay:rand];        
    }
