...
@implementation SyntaxPrimaryView
...
- (id)initWithFrame:(CGRect)frame andViewController:(ViewController *)controller {
    ...
    if (self) {
        ...
        self.gameEngine = [[SyntaxEnginePrimary alloc] initWithFrame:CGRectMake(0, 70, 320, 320) andViewController:viewController];
        ...
        // Levels Mode
        levelsMode = [[SyntaxLabel alloc] initWithFrame:CGRectMake(0, 0, 100, 28)];
        levelsMode.text = @"LEVELS MODE";
        levelsMode.center = CGPointMake(271, 10);
        [levelsMode styleWithSize:12];
        levelsMode.textColor = [UIColor colorWithRed:0.5 green:0.5 blue:0.5 alpha:1];
        [self addSubview:levelsMode];
...
- (void)showButtons {
    ...
    [self fadeInView:levelsMode withAction:^{}];
...
- (void)hideButtons {
    ...
    [self fadeOutView:levelsMode withAction:^{}];
...
- (void)updateScore {    
    ...
    [UIView animateWithDuration:0.3
                     ...
                     animations:^{
                         progressBar.center = CGPointMake(-250 + (300 * k), 9.5);
                     }
                     completion:^(BOOL finished) {                                                                                        
                         //
                     }];
...
- (void)gameOverHandler {
    ...
    [self clearInfoPopUps];
    [self hideButtons];
    [viewController performSelector:@selector(gameOverWithReason:) withObject:@"No Legal Moves Left" afterDelay:1];
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    [self checkIfPowerUpsNeedToGlow];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"refreshStuckTimerPrimary" object:nil];
...
//        (viewController.statsManager.currentLevelScore >= 100) |
        (viewController.byteManager.prompts > 0)) {
