...
@implementation SyntaxEngineInfinity

@synthesize gameGrid, stuckMessage;
...
- (id)initWithFrame:(CGRect)frame andViewController:(ViewController *)controller {
    ...
    if (self) {
        ...
        canShowStuck = YES;
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refreshStuckTimer) name:@"refreshStuckTimerInfinity" object:nil];
...
- (void)swapSymbol:(SyntaxSymbol *)firstSymbol withSymbol:(SyntaxSymbol *)secondSymbol {
    ...
    [UIView animateWithDuration:0.3
                     ...
                     completion:^(BOOL finished) {
                         ...
                         isDoubleMatch = YES;
                         [self searchPatterns];                         
                         if (!isMoveValid) [viewController.infinityView addPoints:-100];
...
- (void)eraseSymbols {
    ...
    [viewController.infinityView addPoints:pointsToAdd];
    [self clearPoints];
...
- (void)searchPatterns {
    ...
    if ([symbolsToRemove count] > 0) {
        ...
        noOfCascadingMatches = 0;
        [self checkScore];
        [viewController.infinityView checkForInfoPopUps];
        canTouch = YES;
    }
    isDoubleMatch = NO;
}
...
- (void)checkScore {
    if (viewController.statsManager.currentLevelScore >= viewController.statsManager.pointsToCompleteLevel) [viewController.infinityView showLevelComplete];
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    [viewController.infinityView checkIfPowerUpsNeedToGlow];
    [self refreshStuckTimer];
    if (canTouch) {
...
- (void)dealloc {
    NSLog(@"infinity engine released");
