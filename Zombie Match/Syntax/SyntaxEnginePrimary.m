...
@implementation SyntaxEnginePrimary

@synthesize gameGrid, canShowStuck;
@synthesize firstTouchedSymbol, stuckMessage;
...
- (id)initWithFrame:(CGRect)frame andViewController:(ViewController *)controller {
    ...
    if (self) {
        ...
        canShowStuck = YES;
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refreshStuckTimer) name:@"refreshStuckTimerPrimary" object:nil];
...
- (void)refreshStuckTimer {
    ...
    timerUntilStuck = [NSTimer scheduledTimerWithTimeInterval:13.0 // 10 plus 3
                                                       target:self
...
- (void)showStuckMessage {
    ...
    stuckMessageCounter++;
    if(stuckMessageCounter == 3) canShowStuck = NO;
...
- (void)checkIfSymbolNotMakingPattern:(SyntaxSymbol *)thisSymbol {
    ...
    if (hasToChange) {
        ...
        NSLog(@"%d to %d", typeToSkip, newType);
    }
}
...
- (void)eraseSymbols {
    ...
    [viewController.primaryView addPoints:pointsToAdd];
    [self clearPoints];
...
- (void)searchPatterns {
    ...
    if ([symbolsToRemove count] > 0) {
        ...
        noOfCascadingMatches = 0;
        [self checkScore];
        [viewController.primaryView checkForInfoPopUps];
        canTouch = YES;
    }
    isDoubleMatch = NO;
}
...
- (BOOL)getHintExceptThis:(CGPoint)lastPoint { // to recognize last move TODO - absorb with next method
    ...
    for (int i = 0; i < 8; i++) {
        for (int j = 0; j < 8; j++) {
            if (!possiblePatternFount && !CGPointEqualToPoint(lastPoint, CGPointMake(i, j))) {
    ...
    [hintGrid release];
    if (possiblePatternFount || !CGPointEqualToPoint(hint, CGPointMake(-1, -1))) return YES;
    else if (foundSpecialSymbol) return YES;
    else return NO;
}
...
- (void)checkScore {    
    if (viewController.statsManager.currentLevelScore >= viewController.statsManager.pointsToCompleteLevel) [viewController.primaryView showLevelComplete];
    else {
        CGPoint hint = [self getHint];
        if(![self getHintExceptThis:hint])
        ...
        if ((hint.x < 0) & (viewController.byteManager.wildcards == 0)) {
            [viewController.primaryView gameOver];
            canShowStuck = NO;
        }
    }
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    [viewController.primaryView checkIfPowerUpsNeedToGlow];
    [self refreshStuckTimer];
    if (canTouch) {
